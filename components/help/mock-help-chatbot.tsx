...
import { Bot, ChevronLeft, ChevronRight, SendHorizonal, Sparkles, X } from 'lucide-react';
...
export function MockHelpChatbot() {
  ...
  const scrollToBottom = useCallback(() => {
    ...
    if (mobileListRef.current) mobileListRef.current.scrollTop = mobileListRef.current.scrollHeight;
  }, []);

  useEffect(() => { scrollToBottom(); }, [messages, thinking, expanded, mobileOpen, scrollToBottom]);

  // Lock scroll + focus input when mobile chat opens
  useEffect(() => {
    if (mobileOpen) {
  ...
  const send = useCallback(() => {
    ...
    setThinking(true);
    window.setTimeout(() => {
      const reply: ChatMessage = { id: nextId(), role: 'assistant', content: mockAssistantReply(trimmed) };
  ...
  const messageList = (ref: React.RefObject<HTMLDivElement | null>) => (
    <div
      ref={ref}
  ...
  return (
    <>
      {/* ══════════════════════════════════════════════════════════════════
          MOBILE — floating button + full-screen overlay (hidden on desktop)
      ══════════════════════════════════════════════════════════════════ */}
      <div className="lg:hidden">
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-50 flex flex-col bg-slate-950">
            <header className="flex shrink-0 items-center justify-between border-b border-slate-800 bg-gradient-to-r from-violet-950/60 to-slate-950 px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Not a real model</p>
                </div>
              </div>
              <button
                type="button"
      ...
      {/* ══════════════════════════════════════════════════════════════════
          DESKTOP — sticky side panel (hidden on mobile)
      ══════════════════════════════════════════════════════════════════ */}
      <aside
        id={panelId}
        style={{ transitionTimingFunction: PANEL_EASE }}
        className={cn(
          'relative sticky top-0 hidden h-screen shrink-0 self-start flex-col overflow-hidden border-l-2 border-l-violet-500/50 border-slate-800 bg-gradient-to-b from-violet-950/35 to-slate-900 lg:flex',
          'transition-[width] duration-300 motion-reduce:transition-none',
          expanded ? 'w-[min(22rem,calc(100vw-9rem))] md:w-96' : 'w-12',
        )}
        aria-label={expanded ? 'IronVault AI chat (prototype)' : 'IronVault AI (collapsed)'}
      >
        {/* Narrow rail */}
        <div
          className={cn(
            'absolute inset-y-0 left-0 z-10 flex w-12 flex-col bg-gradient-to-b from-violet-950/35 to-slate-900 transition-opacity duration-200',
        ...
        >
          ...
          >
            ...
            <span className="select-none text-[10px] font-bold uppercase tracking-[0.12em] text-violet-300/90 [writing-mode:vertical-rl] rotate-180" aria-hidden>
              AI
            </span>
