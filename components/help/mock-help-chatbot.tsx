...
export function MockHelpChatbot() {
  ...
  useEffect(() => {
    scrollToBottom();
  }, [messages, thinking, open, scrollToBottom]);
    ...
    <div className="pointer-events-none fixed bottom-0 right-0 z-50 flex flex-col items-end gap-3 p-4 md:p-6">
      {open ? (
        <div
          ...
          className="pointer-events-auto flex max-h-[min(32rem,calc(100vh-8rem))] w-[min(100vw-2rem,22rem)] flex-col overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl md:w-[26rem]"
        >
          <header className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-semibold text-white">IronVault helper</p>
                <p className="text-xs text-slate-500">Mock assistant · not live AI</p>
              </div>
            </div>
            <button
              type="button"
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="pointer-events-auto flex h-14 w-14 items-center justify-center rounded-full bg-emerald-500 text-slate-950 shadow-lg shadow-emerald-900/40 transition hover:bg-emerald-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950"
        aria-expanded={open}
