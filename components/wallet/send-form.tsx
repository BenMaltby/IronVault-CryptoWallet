...
export function SendForm({ wallets, contacts }: Props) {
  ...
  const isValidRecipientAddress =
    /^0x[a-fA-F0-9]{40}$/.test(formData.recipientAddress.trim());
      ...
      <div className="card p-6">
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleInitiate}>
          ...
          <label className="grid gap-2 md:col-span-2">
            ...
            <input
              className={`rounded-xl border bg-slate-950 px-3 py-2 font-mono text-sm ${formData.recipientAddress && !isValidRecipientAddress
                  ? 'border-rose-500'
                  : 'border-slate-700'
                }`}
              placeholder="0x..."
            ...
            {formData.recipientAddress && !isValidRecipientAddress ? (
              <p className="text-xs text-rose-400">
                Invalid wallet address. It must start with 0x and contain 40 hexadecimal characters.
              </p>
            ) : null}
          </label>
          ...
          <div className="flex items-center justify-between md:col-span-2">
            <button
              type="submit"
              disabled={isSubmitting || !isValidRecipientAddress}
              className="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-70"
            >
